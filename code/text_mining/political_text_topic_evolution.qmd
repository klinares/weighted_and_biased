...
subtitle: "A defensible tidytext + topic-modeling + sentiment workflow validated against a known data-generating process"
author: "Kevin"
date: today
format:
  typst:
...
# Load tidyverse LAST so its dplyr/tidyr verbs win over any same-named functions
# exported by the packages below (e.g. masking of count/filter/lag).
...
library(tidyverse)   # dplyr, tidyr, purrr, ggplot2, stringr, forcats -- LAST

# If a package attached via .Rprofile or the session (e.g. plyr) masks dplyr
# verbs, prefer dplyr. No-op when {conflicted} is not installed; the count()
# calls below are additionally namespaced as dplyr::count() as a safeguard.
if (requireNamespace("conflicted", quietly = TRUE)) {
  conflicted::conflicts_prefer(
    dplyr::count, dplyr::filter, dplyr::lag, dplyr::summarise,
    dplyr::mutate, dplyr::arrange, dplyr::rename, dplyr::select
  )
}
...
$$m_k(t) = v_k + \delta_k (t - \bar t),$$

with intercept $v_k$ and a **topic-specific** slope $\delta_k$. The sentence is drawn positive / neutral / negative from a softmax in $m_k(t)$; positive sentences append a word from a positive bank, negative ones from a negative bank. Both banks are chosen to live in the Bing and AFINN lexicons so the lexical measurement can recover them.
...
Each post's topic mixture $\theta_d$ perturbs the year's prevalence vector on the log scale; each sentence draws a topic from $\theta_d$ and a valence from the topic-and-year valence probabilities. Construction is vectorized with `purrr` (no `for`/`while` loops).
...
# Exploratory text analysis: how emphasis shifts over time

A natural first look would be tf-idf with each *year* as a document,

$$\mathrm{tfidf}(w, y) = \mathrm{tf}(w, y) \cdot \ln\frac{N}{\mathrm{df}(w)},$$

with $\mathrm{tf}(w,y)$ the relative frequency of term $w$ in year $y$ (its share of that year's tokens), $N$ the number of years, and $\mathrm{df}(w)$ the number of years containing $w$. On *this* corpus it is degenerate: the simulated author's vocabulary is stable across the decade, so every term appears in every year, $\mathrm{df}(w) = N$, the factor $\ln(N / \mathrm{df})$ is zero for every term, and tf-idf collapses to zero everywhere — an all-zero plot. That is the correct answer here, and an instructive one: what changes over time in this corpus is not *which* words appear but *how often each topic is discussed*.
...
```{r}
#| label: fig-marker-freq
#| fig-cap: "Share of each year's tokens taken by one marker term per topic — a model-free preview of the planted prevalence shifts."
...
with $\theta_d$ the document-topic mixture (gamma) and $\beta_k$ the topic-word distribution (beta). **LDA has no notion of time**; we estimate per-document topic shares and aggregate by year afterward.
...
$$\theta_d \sim \mathrm{LogisticNormal}(\Gamma' x_d, \Sigma),
\quad x_d = [1, s(\mathrm{year}_d)].$$
...
```{r}
#| label: fig-stm-effect
#| fig-cap: "Formal STM time effect (estimateEffect) with 95% confidence bands for the two non-monotone topics. The spike and bump are recovered with quantified uncertainty."
#| fig-width: 8
#| fig-height: 3.6
...
# topic it does not contain (the cause of the get1index error). Fall back to the
...
## Recovery, measured on the natural scales
...
# Lexicon for the negation diagnostic: AFINN magnitudes when available, else Bing
# collapsed to +/-1. Bing ships with tidytext, so this is always non-NULL and the
# diagnostic runs safely whether or not {textdata} is set up.
neg_lexicon <- if (have_afinn) {
  dplyr::select(afinn, word, value)
} else {
  bing |> transmute(word, value = if_else(sentiment == "positive", 1L, -1L))
}
```

```{r}
#| label: negation-diagnostic
#| fig-cap: "Sentiment words following a negator, weighted by lexicon value x frequency. Empty on the synthetic corpus by construction (no negators planted); shown as the procedure to run on real text."
...
ai_sent <- ai_check |>
  mutate(res = map(text, safe_rate, .progress = "LLM sentiment")) |>
  unnest_wider(res) |>
  left_join(doc_sent_bing, by = c("doc_id", "year"))
...
## Reading the cross-check, and why kappa has no fixed thresholds
...
**Spearman's** $\rho$ is the rank correlation between the LLM's continuous score and the Bing net count:
...
**Cohen's** $\kappa$ measures agreement on the categorical sign (negative / neutral / positive) after removing the agreement expected by chance:
...
- $\kappa$ judges a coarse 3-way sign, and the "neutral" bin is defined by an *exact* zero Bing count — an arbitrary cut that dumps many near-zero posts into disagreement. $\rho$ uses the full continuous signal and is not hostage to it.
...
- $\kappa$ is chance-corrected and so sensitive to the marginal mix: when one category dominates, $p_e$ is large and $\kappa$ is pulled down even when raw agreement $p_o$ is high — the well-known "kappa paradox."

On thresholds: there is **no agreed scale** for what $\kappa$ counts as "good." The familiar Landis-Koch labels ("fair," "moderate," ...) are widely cited but explicitly arbitrary, never validated, and ignore prevalence and bias — exactly what the paradox exploits. So do **not** report "$\kappa = 0.28$, therefore fair agreement" as a finding. Instead report $\kappa$ *with* the raw agreement $p_o$ and the confusion table (both printed above) so the marginal structure is visible; and treat the $z$ / $p$-value as nearly useless here — with $n = 200$ almost any non-zero $\kappa$ is "significant," and significance says nothing about whether the agreement is large enough to matter. The practical reading for this workflow: the LLM and the lexicon corroborate each other enough to be mutually reinforcing but not redundant — which is the case for using the LLM to *catch the negation and context cases the lexicon gets wrong*, not to replace it.
...
# More we can do with AI (local or API, auditable)

The two uses above are deliberately conservative. Several richer applications fit this workflow; they run against a local Ollama server or an API provider (Anthropic, OpenAI) through the same `ellmer` interface, and all remain *measurements to be validated*, not oracles.

::: callout-note
## These examples are inert until you enable them
...
## Aspect-based (topic-conditioned) sentiment
...
## Embedding-based topics and semantic-drift detection
...
emb <- corpus |>
  mutate(vec = map(text, embed_text, .progress = "Embedding posts")) |>
  left_join(doc_dominant, by = c("doc_id", "year"))
...
## Few-shot classification seeded by a codebook
...
ai_topic <- corpus |>
  ...
  mutate(res = map(text, \(txt)
    ...
    .progress = "Codebook classification")) |>
  unnest_wider(res)
...
## Stance detection: position toward a target

Stance is a different construct from sentiment: it is the author's position **toward a stated proposition** (favor, oppose, neutral), not the emotional tone of the text; this favor/against/neither framing follows the stance-detection literature (Mohammad et al. 2016). The two routinely diverge -- "good riddance, half these offices did nothing" is negative in *sentiment* but *favors* workforce cuts -- so a lexicon or sentiment model cannot measure stance, and the target must be named in the prompt. Two further practices come from Li and Conrad (2024): give the model an **irrelevant** option when the corpus was keyword-collected (otherwise it is forced to invent a stance for off-target posts), and expect failure on texts whose stance is implicit -- LLMs disagree with humans exactly where humans disagree with each other, so disputed items should be routed to a human coder rather than trusted.

The chunk below stays clean by sourcing the companion toolkit `llm_classify.R` (same folder as this document), which holds the functions: `stance_rater()` builds the codebook prompt, the constrained label schema, and the chat object from a `"provider/model"` string -- `"ollama/gemma4:31b"` for the local model, or `"openrouter/<slug>"` for any hosted model through OpenRouter (key in `.Renviron` as `OPENROUTER_API_KEY`; slugs at openrouter.ai/models). `classify_texts()` sends the texts concurrently with rate limits, turns failures into NA rows instead of aborting, and can sleep-and-retry through API quota resets. On this simulated corpus there is no planted stance target, so treat this strictly as the procedure for real data.
...
stance_target <- "expanding federal climate regulation"   # name the proposition

# Single local rater: gemma4 via Ollama (`ollama serve` must be running).
stance_raters <- list(
  gemma4 = stance_rater(stance_target, model = "ollama/gemma4",
                        include_irrelevant = TRUE, 
                        params = ellmer::params(
                          temperature = 0.05, seed = 1)))

stance_sample <- corpus |> slice_sample(n = 200)

# max_active = 2 suits a single local GPU; retries = 1, retry_wait = 5 fails
# fast instead of sleeping for hours if the server is unreachable.
ai_stance <- classify_texts(stance_sample, text, stance_raters, id = doc_id,
                            max_active = 2, rpm = 60,
                            retries = 1, retry_wait = 5)

# Label distribution and failure count for the single rater.
ai_stance |> dplyr::count(label)
```

For unattended runs on a quota-limited work API, add `retries = 16, retry_wait = 30 * 60, checkpoint = "stance_run.rds"` to `classify_texts()`: failed texts are retried after a sleep (spanning an 8-hour token reset), and finished models are saved to disk so an interrupted run resumes instead of re-spending tokens.
...
- **Measurement: temporal non-invariance.** LDA and STM assume a *time-invariant* topic-word distribution $\beta$; over a decade, framing drifts, so a fixed $\beta$ can mis-measure later years. Dynamic Topic Models (Blei & Lafferty 2006) relax this; treat decade-long trends as provisional until checked.
...
- Mohammad, S. M., Kiritchenko, S., Sobhani, P., Zhu, X., & Cherry, C. (2016). SemEval-2016 Task 6: Detecting Stance in Tweets. *Proceedings of the 10th International Workshop on Semantic Evaluation (SemEval-2016)*.
- Li, M., & Conrad, F. (2024). Advancing Annotation of Stance in Social Media Posts: A Comparative Analysis of Large Language Models and Crowd Sourcing. arXiv preprint arXiv:2406.07483.
